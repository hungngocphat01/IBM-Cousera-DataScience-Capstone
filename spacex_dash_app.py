...
def pie_chart_generator(site): 
    if site == 'ALL':
        ...
        fig = px.pie(
            ...
            names='class', 
            title='Success Rate for Site %s' % site
        )
    return fig
...
def scatter_callback(site, payload): 
    pl_min, pl_max = payload
    payload_filtered_df = spacex_df[
        (spacex_df['Payload Mass (kg)'] >= float(pl_min)) &
        (spacex_df['Payload Mass (kg)'] <= float(pl_max))
    ] 
    ...
    if site == 'ALL':
        ...
        site_df = payload_filtered_df[payload_filtered_df['Launch Site'] == site]
        fig = px.scatter(
            site_df,
            x='Payload Mass (kg)',
            y='class', 
